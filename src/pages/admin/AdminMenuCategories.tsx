...
const rowToForm = (row: MenuCategoryRow): FormState => ({
...
const AdminMenuCategories = () => {
  ...
  const { data: categories = [], isLoading, isError, error } = useQuery({
    queryKey: ["admin", "menu-categories", statusFilter],
    queryFn: () => fetchMenuCategories(statusFilter),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (editing) {
        return updateMenuCategory(editing.id, payload);
      }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-gradient-gold">Menu Categories</h1>
          <p className="text-sm text-muted-foreground">
            Global categories — visible to all caterers in the app.
          </p>
        </div>
              ...
              <TableBody>
                {filtered.map((row) => (
                  <TableRow key={row.id}>
                    ...
                    <TableCell>{row.menu_items_count}</TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-right space-x-2">
                      ...
                      <Button
                        size="sm"
                        variant="destructive"
                        disabled={row.menu_items_count > 0}
                        onClick={() => {
