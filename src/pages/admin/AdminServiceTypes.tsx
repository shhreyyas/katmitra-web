...
const rowToForm = (row: ServiceTypeRow): FormState => ({
...
const AdminServiceTypes = () => {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (editing) {
        return updateServiceType(editing.id, payload);
      }
      return createServiceType(payload);
    },
    onSuccess: () => {
      toast.success(editing ? "Service type updated" : "Service type created");
      setDialogOpen(false);
      setEditing(null);
      setForm(emptyForm());
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => deleteServiceType(id),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-gradient-gold">Service Types</h1>
          <p className="text-sm text-muted-foreground">
            Global catering service types — caterers select these at registration.
          </p>
        </div>
        <Button onClick={openCreate}>Add service type</Button>
      </div>
                ...
                {filtered.length === 0 ? (
                  ...
                  filtered.map((row) => (
                    <TableRow key={row.id}>
                      ...
                      <TableCell>{row.businesses_count}</TableCell>
                      <TableCell>
